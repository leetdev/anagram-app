...
export default function Home({ wordBases }: Props) {
    ...
    const submit = (e) => {
        ...
        if (word != '') {
            postApi(find.url(), { wordBaseId, word }).then((result) => {
                        ...
                        <div className="flex-1 rounded-br-lg rounded-bl-lg bg-white p-6 pb-12 text-[13px] leading-[20px] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] lg:rounded-tl-lg lg:rounded-br-none lg:p-20 dark:bg-[#161615] dark:text-[#EDEDEC] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d]">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <a
                                        className="ml-2 inline-flex items-center space-x-1 font-medium text-[#f53003] underline underline-offset-4 dark:text-[#FF4433]"
                                        href="/import"
                                    >
                                        Import word list
                                    </a>
                                ...
                                {wordBases.length > 0 ? (
                                    <div>
                                        <input type="text" placeholder="Search" className="w-full border p-2" onChange={submit} />
                                    </div>
                                ) : (
                                    ''
                                )}
                                {anagrams.length > 0 ? (
                                    <div>
                                        <ul className="text-[20px] uppercase">
